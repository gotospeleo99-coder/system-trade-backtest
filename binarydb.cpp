...
int binaryDB::ExportTickerBinary(){//銘柄確認
    ...
    file f;
    std::string fillname = "stockList_"+GetFileDate()+".bin";
...
void List_stocks::StockCodeCheck(std::vector<StockCodeID>& stockCode,std::filesystem::path path){//銘柄コードに対応したindexにIDを埋め込む。　　正常動作確認
    ...
    if (!Latestfile.empty()) {
        ...
        std::vector<std::string> header = List_stocks::ListCSVparse(line);
        int code= -1 ,MktNm = -1;
        int Damageline = 0;
        ...
        while (std::getline(ifs, line)) {//CSVの本文のパース
            std::vector<std::string> row = List_stocks::ListCSVparse(line);
...
std::vector<std::string> List_stocks::ListCSVparse(const std::string& line){
